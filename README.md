...
Discovery is automated on three layers:

- **Sitemap** (`sitemap.xml` / `sitemap-index.xml`): generated by
  `@astrojs/sitemap` with per-page `lastmod` taken from the git history
  (`scripts/lib/git-lastmod.mjs`), not from file mtimes — Vercel's fresh clone
  would otherwise stamp every page with the build time. The Vercel project sets
  `VERCEL_DEEP_CLONE=true` so the full history is available at build time.
  `robots.txt` (`src/pages/robots.txt.ts`) advertises the sitemap.
- **IndexNow** (Bing, Yandex, Seznam, Naver, Yep — DuckDuckGo and Ecosia serve
  from Bing's index): after every successful Vercel *Production* deployment,
  `.github/workflows/indexnow.yml` runs `scripts/indexnow-submit.mjs`, which
  diffs the deployed commit against the last submitted one (state in the
  Actions cache) and pings only the affected URLs. Content-file changes map to
  their routes; template/layout changes trigger a full submission. The API key
  lives as `public/<key>.txt` — the filename is the key, as the protocol
  requires; it is intentionally public.
  Manual runs: `npm run indexnow -- --all --dry-run`, or trigger the workflow
  via `gh workflow run indexnow.yml -f mode=all`.
- **Google** does not support IndexNow; it discovers changes via the sitemap.
  Registering the domain in Google Search Console (and importing it into Bing
  Webmaster Tools from there) remains a manual, account-bound step — see
  Known Launch Checks.
...
- Search-console registrations are account-bound and still open: add
  `kim-marie-borger.com` to **Google Search Console** (Domain property via DNS
  TXT record, or URL-prefix property via HTML tag) and submit `sitemap.xml`
  there; then use **Bing Webmaster Tools → "Import from Google Search
  Console"** to cover Bing with two clicks. IndexNow submission works today
  without either account, but the consoles add indexing diagnostics and
  performance data (`npm run audit:gsc-indexing` waits for exactly that
  export).
